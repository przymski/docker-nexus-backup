...
function backup {
    ...
    tar c "${NEXUS_DATA_DIRECTORY}/blobs/default/" | aws s3 cp - "${TARGET_BUCKET}/${TIMESTAMP}/blobstore.tar"
    ...
    tar c "${NEXUS_BACKUP_DIRECTORY}/" | aws s3 cp - "${TARGET_BUCKET}/${TIMESTAMP}/databases.tar"
